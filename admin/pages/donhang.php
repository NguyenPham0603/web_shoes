...
$sql = "SELECT * FROM hoadon h, khachhang k
        WHERE h.KH_MA = k.KH_MA
        ORDER BY HD_TRANGTHAI ASC, HD_MA ASC
        LIMIT $start, 10";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
            ...
            if($row['HD_TRANGTHAI'] == 1){
                echo "<td style = 'text-align: center; color: red; font-weight: bold;'> Đơn Hàng Mới </td>";
            }
            else if($row['HD_TRANGTHAI'] == 3){
                echo "<td style = 'text-align: center; color:brown;'> Đã Giao Hàng </td>";
            }
        ...
        <td style = 'text-align: center;'>
            <button name = 'xemchitiet'>
                <a href="ad_index.php?act=quanlydonhangchitiet&code=<?php echo $row['HD_MA']; ?>" style = 'text-decoration: none; color: black;'>
                    Xem Chi Tiết
                </a>
            </button>
        </td>
...
<ul class = "list_pages">
    <li>
        <?php list_page('hoadon', 'ad_index.php?act=quanlydonhang&trang=', $connect);?>
    </li>
</ul>
